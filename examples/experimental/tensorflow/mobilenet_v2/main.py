...
def run_example():
    ...
    quantized_model = nncf.quantize(original_model, calibration_dataset)
    ...
    original_results = original_model.evaluate(data_source)
    print(f'accuracy@top1: {original_results[0]:.3f}')
    ...
    quantozed_results = validate(quantized_compiled_model, metrics, data_source)
    print(f'accuracy@top1: {quantozed_results[0]:.3f}')
...
def create_data_source(batch_size: int) -> tf.data.Dataset:
    ...
    :return: The `tf.data.Dataset` object.
    """
