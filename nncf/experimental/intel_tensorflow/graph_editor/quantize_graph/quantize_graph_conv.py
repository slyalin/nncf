...
class FuseNodeStartWithConv2d(QuantizeNodeBase):
    ...
    def __init__(self, **kwargs):
        ...
        self.fusion_mapping = {
            ...
            'Conv2DMulBiasAddAddNRelu6': self.apply_conv_mul_biasadd_addn_relu_fusion,
            'Conv2DMulBiasAddAddV2Relu': self.apply_conv_mul_biasadd_addn_relu_fusion,
            'Conv2DMulBiasAddAddV2Relu6': self.apply_conv_mul_biasadd_addn_relu_fusion,
            'Conv2DMulBiasAddAddRelu': self.apply_conv_mul_biasadd_addn_relu_fusion,
    ...
    def apply_conv_mul_biasadd_addn_relu_fusion(self, match_node_name):
        skip_node_name = match_node_name[1:]
        matched_node = self.node_name_mapping[match_node_name[0]]
        control_inputs, normal_inputs = self._get_node_input(
            matched_node.node.name)
        ...
        for _, node in enumerate(self.input_graph.node):
            if node.name in skip_node_name:
                ...
                sum_index = 1 if match_node_name[2 + leaky_offset] == self.node_name_mapping[
                    match_node_name[3 + leaky_offset]].node.input[0] else 0
                quantized_node_input_names = all_input_names[:2] + [
                    bias_node_name
                ] + all_input_names[2:] + [
                    self.node_name_mapping[
                        match_node_name[3 + leaky_offset]].node.input[sum_index]
                ] + control_inputs
